   ...
   # image=cv2.resize(image,(480,390))
    ...
    for c in cnts:
        ...
        if area==max_rect or area==max_cri:
            ...
            # # # time.sleep(1)
            # cv2.waitKey(45)
    tpl_go= cv2.imread("go.png",0)
    tpl_tr= cv2.imread("tr.png",0)
    tpl_tl= cv2.imread("tl.png",0)
    # print(tpl_go.shape)
    tpl_tb= cv2.imread("./tb.png",0)
    tpl_st= cv2.imread("./st.png",0)
        ...
        if mode==5:
            ...
            if (nest_dist<0.32 and rearfront==0) or (nest_dist<0.38 and rearfront==1):
                ...
                def laser_callback(laser):
                    global  low_range, up_range 
                    # 计算角度
                    for j in range(low_range, up_range, 4):
                        i = (j + 720) % 1440

                        # 检测最近的P
                        if laser.ranges[i] < nearest_dist:
                            tt += 1
                            if tt == 3:
                                nearest_dist = laser.ranges[i]
                                far_dist=nearest_dist
                                tt = 0
                            
                        else:
                            tt = 0
                        #检测较远的P
                        if laser.ranges[i] >far_dist:
                            tt += 1
                            if tt == 3:
                                far_dis = laser.ranges[i]
                                tt = 0
                                
                        else:
                            tt = 0
                #PC1=[0,0]                                                                                               #记录中心坐标PC1默认为(0,0)                                                        
                D1=30 #参数调整                                                               #与P1坐标距离D1
