...
key_bindings = {
    # The STM32 protocol used by this car treats negative speed as forward.
    '\x77' : (-1.0 , 0.0),
    '\x73' : ( 1.0 , 0.0),
    '\x64' : ( 0.0 ,-1.0),
    '\x61' : ( 0.0 , 1.0),
...
class AckermannDriveKeyop:
    ...
    def print_state(self):
        ...
        rospy.loginfo('\x1b[1M\rUse W/S for speed and A/D for steering')
        rospy.loginfo('\x1b[1M\rUse space to brake and tab to align wheels')
        rospy.loginfo('\x1b[1M\rPress <ctrl-c> or <q> to exit')
        rospy.loginfo('\x1b[1M\r*********************************************')
        rospy.loginfo('\x1b[1M\r'
                      '\033[34;1mSpeed command: \033[32;1m%0.2f, '
                      '\033[34;1mSteer command: \033[32;1m%0.2f\033[0m',
        ...
        while 1:
            ...
            if key in key_bindings.keys():
                if key == control_keys['space']:
                    ...
                    self.steering_angle = \
                            int(self.steering_angle + key_bindings[key][1])
                    self.speed = clip(
