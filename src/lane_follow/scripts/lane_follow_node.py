...
class Trial(object):
    def __init__(self, args):
        ...
        self.controller = SteeringController(self.cfg)
        ...
        if os.path.exists(self.out):
            raise RuntimeError('Output directory exists; use a new run name')
    ...
    def run(self):
        ...
        if scan_topic not in topics:
            ...
            self.spawn('lidar', ['rosrun', 'ls01b_v2', 'ls01b_v2', '__name:=lane_follow_lidar',
        ...
        while not self.exit_requested and not rospy.is_shutdown():
            ...
            control = self.controller.update(result, frame_time)
            if control is not None:
                result.update(control)
            elif result['valid']:
                result.update(valid=False, reason='controller_timestamp_invalid')
            overlay = self.detector.draw(cv2.resize(frame, (640, 480)), result)
            now = time.time()
            ...
            if overlay is not None:
                if self.video is None:
                    ...
                    self.raw_video = cv2.VideoWriter(os.path.join(self.out, 'raw.avi'),
                        cv2.VideoWriter_fourcc(*'MJPG'), 10., (640, 480))
                    if not self.video.isOpened() or not self.raw_video.isOpened():
                        raise RuntimeError('Video recording could not be opened')
                self.video.write(overlay)
                self.raw_video.write(cv2.resize(frame, (640, 480)))
                if frames % 5 == 0:
...
if __name__ == '__main__':
    ...
    if args.config is None:
        args.config = os.path.join(rospkg.RosPack().get_path('lane_follow'),
